...
Follow the steps for link:/documentation/PineNote/Development/Flashing#Creating a mainline boot image|Creating a mainline boot image[Creating a mainline boot image], but instead of flashing it with _rkdeveloptool_, use _fastboot_. You can enter fastboot in either of two ways:
