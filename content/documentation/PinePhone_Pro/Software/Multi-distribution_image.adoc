...
[TIP]
====
The multi-boot image _can_ function without rk2aw, but you will need to make sure the correct <<uboot,bootloader>> is started by your phone's SoC.
...
NOTE: This is the same `ppp.tar.gz` from the <<rk2aw>> section; if you already have a copy of it on your computer then you don't have to download it again.
