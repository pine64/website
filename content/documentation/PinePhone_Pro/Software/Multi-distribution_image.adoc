...
sudo sfdisk /dev/$DEVICE --wipe always <<EOF
 ...
 # attrs="RequiredPartition,LegacyBIOSBootable", size=$PARTSIZE, name="ppp-multi-image-sailfish" # not used
 # attrs="RequiredPartition,LegacyBIOSBootable", size=$PARTSIZE, name="ppp-multi-image-ut" # not used
...
Replace/create `/boot/extlinux/extlinux.conf`, making sure you remove `#` comment for needed distribution:
...
Replace/create `/etc/fstab`, making sure you remove `#` comment for needed distribution:
