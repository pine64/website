...
Connect your PinePhone Pro to a Linux computer amd press power button on. From the graphical menu select `eMMC over USB` or `SD over USB` to expose the device to your Linux computer. Find out the device name using the command `lsblk` and then make sure there are no signatures or partitions left with the command `sudo wipefs /dev/$DEVICE`. To erase all signatures, type `sudo wipefs --all --force /dev/$DEVICE`.
