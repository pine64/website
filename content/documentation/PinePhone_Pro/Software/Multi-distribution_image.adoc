...
This article explains how to install a multi-distribution image to the microSD card.

== Prerequisit: rk2aw pre-loader on SPI

In case your Pinephone Pro doesn't have rk2aw pre-loader you must first istall it. Connect PinePhone Pro to a Linux machine (USB cable or wifi connection is fine). Make sure phone's ssh server is up and running, and find the target IP address. Start a ssh connection from PC and download from the web the pre-loader installer to flash into the phone's SPI.
