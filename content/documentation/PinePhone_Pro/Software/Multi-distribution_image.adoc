...
==== Arch, Manjaro, Mobian, postmarketOS distributions need following steps.
...
Mount the image for the selected distribution:
...
Copy the extracted files directly into the device:
...
Then write the new `/etc/fstab` file, making sure you remove `#` comment for selected distribution:
...
On the first boot, if it doesn't happen automatically, you can manually resize each image to fill the entire partition using GParted GUI software or using the CLI. Pls note that Sailfish doesn't need any resizing.
