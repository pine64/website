...
You can optionally remove bootloader in SPI and flash U-Boot in front of first sectors of your microSD card. This step is not reccomended, and is mentioned just for experimetal purposes.
...
If you removed bootloader (as per experimetal purposes) from PinePhone Pro's SPI, write U-Boot on first sectors of microSD card.
