...
You can optionally remove bootloader in SPI and flash U-Boot in front of first sectors of your SD card
...
If you removed bootloader from PinePhone Pro's SPI, write U-Boot on first sectors of SD card.

 cd ~/rk2aw/ppp/rk2aw
 sudo dd if=ppp/foss/u-boot-rockchip.bin of=/dev/sdb bs=512 seek=64
 sudo sync
