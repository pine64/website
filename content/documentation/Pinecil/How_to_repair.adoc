...
. *Temperature is flickering wildly*: Most likely just need to clean whole cartridge/tip with IPA, see link:#Poor_Contact_Fix[Poor Contact Fix here]. If it's jumping wildly after reaching set temperature, this is also caused by using a low amp/volt charger that is below the 3amp minimum recommended for pinecil, upgrading to a 3.25amp/20V charger often fixes this. Also might need to update firmware; there are ADC adjustments coming in 2.21 Release IronOS.
...
. *I hear a sizzling/crackle sound from my new Pinecil*: this is usually fine and may disappear after a few days of use, see [[#Hissing_Crackle_Sound|Hissing Crackle Sound]].
...
. This modification is not for the V2 as the newer V2 already has 24V DC capability.
