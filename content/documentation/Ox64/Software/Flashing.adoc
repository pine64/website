...
On the `ttyACM0` console you will see Linux booting up. When prompted, log in as `root` with no password. In case card is missing or empty, you'll get `Card did not respond to voltage select! : -110` error.

On the `ttACM1` console you'll see following log, until the sytem is fully loaded.
...
Wait for it to connect (if you're already watching the M0 console on `/dev/ttyACM1` it should tell you there), then run the following command from the Linux console.
