...
Sice the Ox64 wifi scan doesn't always work properly, run the connection command from D0/Linux on /dev/ttyACM0 serial console:

 blctl connect_ap YourAP YourPassword

Wait for it to connect (if you're already watching the M0 on /dev/ttyACM1 serial console it should tell you there), then:
