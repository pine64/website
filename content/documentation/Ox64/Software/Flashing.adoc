...
Check that you serial adapter is on the link:/documentation/Ox64/Further_information/Compatible_UARTs/[Compatible_UARTs] list. You will (most likely) only have one serial interface available to you.
...
finding the correct device path to your serial adapter under `ls /dev/ttyUSB*`.
...
First, write the normal Linux image to the SD card if you have not done so already. Find the correct device under `lsblk`:
