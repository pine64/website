...
First, download the Raspberry Pi Pico firmware that allows it to act as a serial adapter. You may skip this step if you are using your own serial adapter (however see the page link:/documentation/Ox64/Further_information/Compatible_UARTs/[Compatible UARTs] and link:#using_a_serial_adapter_other_than_the_pico[Using a serial adapter other than the Pico]).
...
image:/documentation/Ox64/images/ox64_serial_pipico.png[Ox64 serial PiPico,title="Ox64 serial PiPico", 300]
